...
let HttpAgent = class HttpAgent extends http.Agent {
    /**
   ...
   */ setReqProps(req, opts) {
        ...
        var _opts_pathname;
        const path2 = `${opts.protocol}//${opts.hostname}${port}${(_opts_pathname = opts.pathname) != null ? _opts_pathname : ''}`;
        log({
            path: req.path,
            path2
        }, 'setReqProps');
   ...
   */ createConnection(opts, cb) {
        ...
        if (!proxy) {
            ...
            /** @type {net.Socket} */ const socket = net.createConnection(opt);
            socket.once('connect', ()=>{
            ...
            if (!_.tunnel) {
                ...
                if ([
                    ...
                    Promise.resolve()// connect 内部实现需要确保处理了 proxyOpts
                    .then(()=>connect(opts, this.proxy, this.proxyOpts))// @ts-ignore
                    .then(({ socket, err })=>{
                        if (socket) {
   ...
   */ constructor(opts){
        ...
        if (px) {
            ...
            this.tunnel = tunnel != null ? tunnel : false;
